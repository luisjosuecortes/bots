...
TF = "1h"
SOURCE = "binance"           # el mercado 1h SE RESUELVE con Binance BTC/USDT: usamos la misma
FUENTE = "Binance BTC/USDT (vela 1H)"
TABLA = {}
MIN_LIQUIDEZ = 5000.0
...
REGIMEN_LOOKBACK_MIN = 360   # 6h de tendencia para el régimen
REGIMEN_MIN_PCT = -2.0       # solo pausa en free-fall REAL (seguro de cola ante un bear sostenido)
DIAS_HISTORICOS = 30
REFRESH_HORAS = 24
...
async def obtener_mercado(session):
    """Datos del mercado simulando el fill REAL de una orden de TAMANO_ORDEN (VWAP del libro)."""
    try:
        slug = generar_slug()
...
async def main():
    ...
    TABLA = ejecutar_analisis_temporal(interval="1h", days=DIAS_HISTORICOS, source=SOURCE)

    CSV_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "senales_1h.csv")

    init(TF, f"filtros: liquidez≥${MIN_LIQUIDEZ:,.0f} · orden {TAMANO_ORDEN:.0f}u · EV≥{MARGEN_EV_MINIMO*100:.0f}% · fee {FEE:.2f} · ventana 1h")
    init(TF, f"fuente de resolución: {FUENTE} (coincide con nuestra calibración)")
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            try:
                ...
                prob = probabilidad_temporal(TABLA, "1h", elapsed_min, caida_pct)
